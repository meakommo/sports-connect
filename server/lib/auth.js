...
function isUser (req, res, next) {
  const user = req.user
  if (!userPermissions.includes(user.role)) {
...
function isUserOrAdmin (req, res, next) {
  const user = req.user
  if (!(userPermissions.includes(user.role) || adminPermissions.includes(user.role))) {
    return res.status(403).json({errorMessage: 'User not authorised'})
  }
  next()
}
...
function isModerator (req, res, next) {
  const user = req.user
  if (!moderatorPermissions.includes(user.role)) {
    return res.status(403).json({errorMessage: 'User not authorised'})
  }
  next()
}
